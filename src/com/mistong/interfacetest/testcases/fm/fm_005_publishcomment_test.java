...
/**
 * @接口 /api/request.do?msgId=13005
 * @描述 心理FM发表评论/回复
 * @author wudingfei
 * 
 */

public class fm_005_publishcomment_test extends BaseParpare {
	String memberId = null;
	String topicId = null;
	int i;
	String parentId = null;
	// 输出本页面日志 初始化
	static Logger logger = Logger.getLogger(fm_005_publishcomment_test.class
			.getName());
	...
	public void startTest(ITestContext context) {
		...
		logger.info("清除心理FM评论/回复数据，请等待......");
		String del1;
		del1 = "delete from fm_posts_info where date_format(CREATE_DATE,'%y-%m-%d') = curdate() ";
		DBHelper.executeNonQuery("ikukocesi", del1);
		logger.info("清除心理FM评论/回复数据完成");
	...
	public void testCase(Map<String, String> data) {
		...
		if (i == 5) {
			// 查询主题帖回复内容的parentId
			String qsql3 = "select id from fm_posts_info where date_format(CREATE_DATE,'%y-%m-%d') = curdate() and member_id = "
					+ memberId;
