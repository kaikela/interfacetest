...
/**
 * @接口     /api/request.do?msgId=03003
 * @描述               所有课程
 * @author  wudingfei
 *
 */
public class AllCourses_001_pass_test extends BaseParpare {
	String memberId = null;
//	int i = 0 ;
	// 输出本页面日志 初始化
	static Logger logger = Logger.getLogger(AllCourses_001_pass_test.class
			.getName());
	...
	public void testCase(Map<String, String> data) {
		...
		UnirestUtil.InterfaceTest(url, StrToMap(data.get("msgId")),
				StrToJson(data.get("json").replace("{memberId}", memberId)), data.get("expect"));
				System.out.println("接口ID"+":"+data.get("msgId"));
//		i = i + 1;
		/********************************************************************************
...
//					+ "WHERE t.id = t2.commodity_id AND t.id = t3.commodity_id AND t.TEACH_ID = t4.ID AND t.SUBJECT_ID = 11 AND t.grade = '0000000010000000' "
