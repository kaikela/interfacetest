...
/**
 * @接口 /api/request.do?msgId=11004
 * @描述 订单取消
 * @author wudingfei
 * 
 */
public class payment_004_ordercancel_test extends BaseParpare {
	String memberId = null;
	int i = 0;
	String orderNo, commodityName, totalPrice, outTradeNo, OrderInfo;
	// 输出本页面日志 初始化
	static Logger logger = Logger.getLogger(payment_004_ordercancel_test.class
			.getName());
	...
	public void testCase(Map<String, String> data) {
		...
		case 1:// 取消未支付的订单，
			...
			OrderInfo = UnirestUtil.PostJson(
					...
					StrToJson("commodityId:333,memberId:" + memberId
							+ ",payType:1")).toString();
			// 获取订单信息
						orderNo = UnirestUtil.GetJsonValue(OrderInfo, "data.orderNo");
			// 执行接口测试
			UnirestUtil.InterfaceTest(url, StrToMap(data.get("msgId")),
					StrToJson(data.get("json").replace("{orderNo}", orderNo).replace("{memberId}", memberId)),
					data.get("expect"));
			break;
		case 2:// 取消已支付的订单
			// 生成订单
			OrderInfo = UnirestUtil.PostJson(
					"http://mobile.kaike.la/app/api/request.do",
					StrToMap("msgId:11001"),
					StrToJson("commodityId:336,memberId:" + memberId
							+ ",payType:1")).toString();
			// 获取订单信息
			orderNo = UnirestUtil.GetJsonValue(OrderInfo, "data.orderNo");
			// 获取商品名称
