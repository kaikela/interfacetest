...
/**
 * @接口 /api/request.do?msgId=12010
 * @描述 回复点赞
 * @author wudingfei
 * 
 */

public class circle_010_praisereply_test extends BaseParpare {
	String memberId = null;
	String topicId = null;
	int i;
	String replyId = null;
	// 输出本页面日志 初始化
	static Logger logger = Logger.getLogger(circle_010_praisereply_test.class
			.getName());
	...
	public void startTest(ITestContext context) {
		...
		UnirestUtil.PostJson("http://10.0.0.224:4080/app/api/request.do",
				StrToMap("msgId:12008"), StrToJson("topicId:" + topicId
						+ ",parentId:0,memIdEd:" + memberId
						+ ",content:test,memberId:3023408,isSecretInfo:0"));
	...
	public void testCase(Map<String, String> data) {
		...
		UnirestUtil.InterfaceTest(url, StrToMap(data.get("msgId")),
				StrToJson(data.get("json").replace("{memberId}", memberId)
						.replace("{replyId}", replyId)), data.get("expect"));
