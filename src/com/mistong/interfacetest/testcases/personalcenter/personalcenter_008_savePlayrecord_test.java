package com.mistong.interfacetest.testcases.personalcenter;

import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.Map;
import java.util.UUID;

import org.apache.log4j.Logger;
import org.testng.ITestContext;
import org.testng.annotations.BeforeClass;
import org.testng.annotations.Test;

import com.mistong.interfacetest.util.DBHelper;
import com.mistong.interfacetest.util.UnirestUtil;
import com.mistong.interfacetest.base.BaseParpare;

/**
 * @接口     /api/request.do?msgId=05008
 * @描述              保存个人播放记录
 * @author  YY
 * 
 */
public class personalcenter_008_savePlayrecord_test extends BaseParpare {
	String memberId = null;
	String uuid = null;
	// 输出本页面日志 初始化
	static Logger logger = Logger
			.getLogger(personalcenter_008_savePlayrecord_test.class.getName());

	@BeforeClass
	/**接口测试前准备工作*/
	public void startTest(ITestContext context) {
		moduleName = getModuleName();
		functionName = getFunctionName();
		caseNum = getCaseNum();
		logger.info(moduleName + "模块" + functionName + "功能" + caseNum
				+ "接口测试开始");
		String qsql1 = "select id from T_MEMBER where LOGIN_NAME= 'yy0011'";
		// 查询
		ResultSet qrs1 = DBHelper.executeQuery("ikukocesi", qsql1);
		// 输出
		try {
			while (qrs1.next()) {
				memberId = qrs1.getString(1);				
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		System.out.println("memberID"+":"+memberId);	
		
		 uuid = UUID.randomUUID().toString();
		 
		 System.out.println("UUID"+":"+uuid);
		 
	}


	@Test(dataProvider = "testData")
	public void testCase(Map<String, String> data) {
		
		
		// 进行用户登录操作
		UnirestUtil
				.PostJson(
						"http://mobile.kaike.la/auth/backend_login.do",
						StrToMap("loginName:yy0011,password:e10adc3949ba59abbe56e057f20f883e"),
						StrToJson("null"));
		
		//进行接口测试，并验证测试结果
		 System.out.println("UUID"+":"+uuid);
//			UnirestUtil.InterfaceTest(url, StrToMap(data.get("msgId")),
//					StrToJson(data.get("json").replace("{memberId}", memberId)),
//					data.get("expect"));		 
				 String string=data.get("json");
				 String memberId_new = string.replace("{memberId}", memberId);
				 System.out.println(memberId_new);
				 String uuid_new = string.replace("{uuid}", uuid);		 
				 System.out.println(uuid_new);	
	
				 String res = data.get("json").replace("{memberId}",memberId).replace("{uuid}", uuid);
				 	UnirestUtil.InterfaceTest(url, StrToMap(data.get("msgId")),
				 			StrToJson(res),
				 			data.get("expect"));
//				 	System.out.println();

//				UnirestUtil.InterfaceTest(url, StrToMap(data.get("msgId")),
//				StrToJson(data.get("json").replace("{uuid}", uuid)),
//				data.get("expect"));

		
	}
	
	
}
